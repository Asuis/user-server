...
class TunnelAPI {
	...
	public com.qcloud.weapp.tunnel.Tunnel requestConnect(String receiveUrl) throws Exception {
		...
		com.qcloud.weapp.tunnel.Tunnel tunnel = new com.qcloud.weapp.tunnel.Tunnel(result.getString("tunnelId"));
		tunnel.setConnectUrl(result.getString("connectUrl"));

		return tunnel;
	}

	public EmitResult emitMessage(com.qcloud.weapp.tunnel.Tunnel[] tunnels, String messageType, Object messageContent) throws com.qcloud.weapp.tunnel.EmitError {
	...
	public EmitResult emitMessage(String[] tunnelIds, String messageType, Object messageContent) throws com.qcloud.weapp.tunnel.EmitError {
	...
	public EmitResult emitPacket(String[] tunnelIds, String packetType, JSONObject packetContent) throws com.qcloud.weapp.tunnel.EmitError {
	...
	public JSONObject request(String path, Object data, Boolean isSendTcKey) throws Exception {
		...
		try {
			...
			if (body.getInt("code") != 0) {
				throw new Exception(String.format("信道服务调用失败：#%d - %s", body.getInt("code"), body.get("message")));
			}
